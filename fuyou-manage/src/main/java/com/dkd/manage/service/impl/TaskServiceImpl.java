...
public class TaskServiceImpl implements ITaskService {
    ...
    public int insertTask(TaskDto taskDto) {
        ...
        // 6.1 判断是不是维修工单，是的话保存成功更新设备状态为待维修
        if (taskDto.getProductTypeId().equals(DkdContants.TASK_TYPE_REPAIR)){
            int update = vendingMachineMapper.updateVendingMachine(new VendingMachine()
                    .setVmStatus(DkdContants.VM_STATUS_REPAIR)
                    .setId(vendingMachine.getId()));
            if (update < 0) {
                throw new ServiceException(MessageFormat.format(MessageConstants.UPDATE_VM_STATUS_FAILD, taskDto.getInnerCode()));
            }
        }
        // 7. 判断是否为补货工单
